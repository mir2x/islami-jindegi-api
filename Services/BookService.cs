    ...
    public async Task<IEnumerable<BookAuthorOption>> GetAuthorsAsync(bool published, string? search = null, int? page = null, int? pageSize = null)
    {
        ...
        var projected = query
            .Select(a => new
            {
                a.Id,
                a.Name,
                Count = a.Books.Count(b => b.Published == published),
                Position = a.Modules
                    .Where(m => m.Module == AuthorModules.Book)
    ...
    public async Task<IEnumerable<BookCategoryOption>> GetCategoriesAsync(bool published, string? search = null, int? page = null, int? pageSize = null)
    {
        ...
        var projected = query
            .Select(c => new
            {
                c.Id,
                c.Title,
                Count = c.Books.Count(b => b.Published == published),
                Position = c.Modules
                    .Where(m => m.Module == ContentModules.Book)
                    .Select(m => (int?)m.Position)
                    .FirstOrDefault()
            })
            .Where(c => c.Count > 0)
